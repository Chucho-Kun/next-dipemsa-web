...
> Este spec adapta `specs/ejemplo-loader-compra-exitosa.md`, aprobado en un e-commerce hermano, a este repositorio (DIPEMSA). El código de checkout es prácticamente idéntico entre ambos proyectos — todos los números de línea del original coinciden 1:1 con este repo — así que el diagnóstico y el plan se conservan. Los ajustes reales fueron: los nombres de las claves de `localStorage` (aquí son `dipemsa-*`, no `ferredip-*`), sumar `app/(public)/compra/pago-exitoso/page.tsx` al alcance para alinear su spinner de `Suspense` con el color del overlay, y documentar una cuarta fuente de re-render (`MediosdePagoComponent.tsx`) que queda cubierta sin tocarla.
...
**Nota propia de este repo:** `MediosdePagoComponent.tsx:12-13` (`const { formData, validateForm } =
useDeliveryStore();` / `const { items, totalPrice, subTotal, shippingCost } = useCartStore();`) es una
cuarta fuente de re-render, no mencionada en el spec original: es el padre de `MercadoPagoButton`, así
que teclear en Entrega también lo re-renderiza a él. No hace falta tocarlo: una vez aplicados los pasos
3 y 4 de este plan, `initialization`, `onSubmit`, `onReady` y `onError` —las props que el `useEffect`
interno del SDK observa— quedan con referencia estable pase lo que pase arriba en el árbol, así que ese
re-render ya no llega a producir un remonte del Brick.
...
- `src/shared/components/cart/MercadoPagoButton.tsx`: agregar `'use client'` (hoy funciona por
  herencia de `MediosdePagoComponent.tsx`, pero un archivo con hooks debe declararlo explícitamente) y
  memoizar `onSuccess` con `useCallback`.
- `app/(public)/compra/pago-exitoso/page.tsx`: alinear el fallback del `Suspense` (hoy spinner
  `border-orange-600` + "Procesando tu confirmación...") con el color y el copy del overlay, para que
  no haya un parpadeo naranja entre el overlay verde y el check verde de la página de destino.
- Se conserva el `setTimeout` de 1 s antes del redirect.

**Fuera de alcance (para specs futuros):**

- `MediosdePagoComponent.tsx` — no se cambia a selectores de zustand; ver nota en "Por qué existe este
  spec" sobre por qué no hace falta.
- El `alert('Error al procesar el pago')` nativo y bloqueante del `catch` en
  `MercadoPagoBrick.tsx:261` — es un problema real (rompe el flujo con un diálogo del navegador) pero
  independiente del remonte del Brick; no se toca aquí.
...
- Rediseñar el resto de la página `/compra/pago-exitoso` (más allá del fallback del `Suspense`) o
  darles contenido real a las páginas stub `pago-fallido` y `pago-pendiente`.
...
- Unificar los cuatro spinners distintos que hoy conviven (`LoadingComponent.tsx:6`, la copia inline de
  `ResumenCompra.tsx:69-73`, y el `Loader2` de `MercadoPagoButton.tsx:56`) — el del fallback de
  `pago-exitoso/page.tsx` sí se toca (ver Dentro), los otros tres no.
...
Este spec **no introduce estructuras en la base de datos** ni claves nuevas en `localStorage`. Las
tres claves que ya existen en este proyecto siguen exactamente igual, sin renombrar:

- `dipemsa-cart` (`src/store/cartStore.ts:105`, vía `persist` de zustand) — items del carrito.
- `dipemsa-last-order` (`src/utils/orderSnapshot.ts:3`) — snapshot de la orden para el evento GA4
  `purchase` en `/compra/pago-exitoso`.
- `dipemsa-purchase-sent` (`src/utils/orderSnapshot.ts:4`) — ids de pago ya reportados, para
  deduplicar ese evento.
...
1. **Componente del overlay.** Crear `src/shared/components/cart/PagoProcesadoOverlay.tsx`
   ...
   con flex, tarjeta con ícono (palomita en círculo verde `#16a34a` para `aprobado` — el mismo verde
   que ya usa `PagoExitosoContent.tsx:104` en su `CheckCircle`, para dar continuidad visual entre el
   overlay y la página de destino —, reloj ámbar para `pendiente`), título, cuerpo y, en el pie, un
   `Loader2` de `lucide-react` con `animate-spin` junto al texto de redirección. Un `useEffect` pone
   `document.body.style.overflow = 'hidden'` al montar y lo restaura al desmontar. `role="status"` y
...
3. **`'use client'` + memoizar `onSuccess`.** En `MercadoPagoButton.tsx`, agregar la directiva
   `'use client'` al inicio del archivo y envolver el handler en `useCallback` con dependencias
   vacías:
...
4. **Dejar de suscribirse a los stores completos.** En `MercadoPagoBrick.tsx`, quitar el destructuring
   ...
   `[onSuccess, handleReset]` y `handleReady` con `[]` —todas estables— y el `setFormData({ email })`
   de la línea 110 deja de provocar un re-render del Brick (se conserva la llamada; simplemente ya no
   se escucha). Esto también neutraliza la cuarta fuente de re-render descrita arriba
   (`MediosdePagoComponent.tsx:12-13`): aunque ese padre siga re-renderizando por cada tecla en
   Entrega, ya no le queda ninguna prop inestable que pasarle al Brick.
...
6. **Alinear el fallback de `/compra/pago-exitoso` y cerrar.** En
   `app/(public)/compra/pago-exitoso/page.tsx`, cambiar el fallback del `Suspense` (hoy
   `border-orange-600` + "Procesando tu confirmación...") para que use el mismo verde `#16a34a` y un
   copy consistente con el overlay, de modo que la transición entre ambos no se note. Quitar los
   `console.count`/renders temporales y correr `npm run lint` y `npm run build`.
   *Verificación visual:* con tarjeta APRO, de principio a fin del flujo no debe verse ningún destello
   naranja entre el overlay y el contenido final de `/compra/pago-exitoso`.
...
- [ ] Ese overlay sigue visible durante toda la navegación a `/compra/pago-exitoso` hasta el primer
      paint del documento nuevo — en ningún momento se ve el formulario de Mercado Pago vacío o
      reiniciándose, ni el fallback naranja original del `Suspense` (ver siguiente criterio).
- [ ] El fallback del `Suspense` de `/compra/pago-exitoso` usa el mismo verde y un copy consistente
      con el overlay, no el spinner naranja original.
...
- [ ] El evento GA4 `purchase` se sigue disparando una sola vez en `/compra/pago-exitoso` y el carrito
      se sigue vaciando ahí; `dipemsa-last-order` y `dipemsa-purchase-sent` siguen funcionando igual.
...
- **Sí:** verde `#16a34a` para el overlay (en vez del naranja de marca `#FF5E00` o el azul `#1E2A44`
  de otros elementos del checkout). Da continuidad con el `CheckCircle` verde que ya usa
  `PagoExitosoContent.tsx:104`, así que el usuario ve el mismo lenguaje visual antes y después de la
  redirección.
- **Sí:** alinear también el fallback del `Suspense` de `pago-exitoso/page.tsx`. Es un archivo extra
  respecto al spec original, pero sin este cambio el usuario vería overlay verde → destello naranja →
  check verde, justo el tipo de parpadeo que este spec busca eliminar.
...
- **No:** tocar `MediosdePagoComponent.tsx` para usar selectores de zustand. Es una cuarta fuente de
  re-render real, pero una vez que `initialization`/`onSubmit`/`onReady`/`onError` quedan estables
  (pasos 3 y 4), un re-render del padre ya no provoca remonte del Brick — no hace falta ampliar la
  superficie de cambio para resolver un síntoma que ya desaparece.
...
- **No:** tocar el `alert()` nativo del `catch` en `MercadoPagoBrick.tsx:261`. Es un problema real
  (bloquea la pestaña con un diálogo del navegador) pero independiente del remonte del Brick y del
  overlay; se deja para otro spec.

## Riesgos

| Riesgo | Mitigación |
|---|---|
| Quitar la suscripción a los stores rompe algún consumo no evidente de esos datos en el Brick | Solo `handleSubmit` y `handleReady` los usan (verificado en las líneas 100-278 de este repo); ambos los leen dentro del evento, donde `getState()` devuelve exactamente lo mismo. |
| El re-render que baja de `MediosdePagoComponent.tsx:12-13` sigue produciendo remonte | Solo pasa si los pasos 3 y 4 se aplican por separado o quedan a medias — con ambos hechos, todas las props que el `useEffect` interno del SDK observa quedan estables sin importar cuántas veces se re-renderice el padre. |
...
| `z-[100]` compite con el `<Toaster position="top-center" />` (`app/layout.tsx:35-37`) o con el iframe del Brick | Se verifica en pantalla que el overlay quede encima del formulario; los toasts de error solo ocurren cuando el overlay no está montado. |
...
- Rediseñar el resto de `/compra/pago-exitoso` ni dar contenido a `pago-fallido` / `pago-pendiente`.
- Unificar los tres spinners duplicados restantes del proyecto (`LoadingComponent.tsx`,
  `ResumenCompra.tsx`, `MercadoPagoButton.tsx`).
- Persistir la orden en base de datos.
- Cambiar `MediosdePagoComponent.tsx` a selectores de zustand.
- El `alert()` nativo del `catch` en `MercadoPagoBrick.tsx:261`.
